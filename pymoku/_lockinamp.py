...
REG_LIA_INT_FBGAIN	= 99
REG_LIA_INT_PGAIN	= 100
REG_LIA_DIFF_DGAIN	= 101
REG_LIA_DIFF_PGAIN	= 102
REG_LIA_DIFF_IGAIN	= 103
REG_LIA_DIFF_IFBGAIN	= 104
REG_LIA_DEMODFREQ_L	= 105
REG_LIA_DEMODFREQ_H	= 106
REG_LIA_PHASEOFF_L	= 107
REG_LIA_PHASEOFF_H	= 108
...
_LIA_SINE_FS		= 1e9
_LIA_COEFF_WIDTH	= 16
_LIA_FREQSCALE		= _LIA_SINE_FS / 2**48
_LIA_PHASESCALE		= 1/2**48
...
class LockInAmp(_frame_instrument.FrameBasedInstrument):
	...
	def __init__(self):
		...
		super(LockInAmp, self).__init__(_frame_instrument.VoltsFrame, scales=self.scales)
		self.id = 7
		self.type = "LockInAmp"
...
_lia_reg_hdl = [
	('pid1_en',		REG_LIA_ENABLES,		lambda s, old: (old & ~1) | int(s),
											lambda rval: rval & 1),
	('pid2_en',		REG_LIA_ENABLES,		lambda s, old: (old & ~2) | int(s),
											lambda rval: rval & 2 >> 1),
	('pid1_int_i_en',		REG_LIA_ENABLES,	lambda s, old: (old & ~2**2) | int(s),
											lambda rval: rval & 2 >> 2),
	('pid2_int_i_en',		REG_LIA_ENABLES,	lambda s, old: (old & ~2**3) | int(s),
											lambda rval: rval & 2 >> 3),
	('pid1_int_p_en',		REG_LIA_ENABLES,	lambda s, old: (old & ~2**4) | int(s),
											lambda rval: rval & 2 >> 4),
	('pid2_int_p_en',		REG_LIA_ENABLES,	lambda s, old: (old & ~2**5) | int(s),
											lambda rval: rval & 2 >> 5),
	('pid1_diff_d_en',	REG_LIA_ENABLES,	lambda s, old: (old & ~2**6) | int(s),
											lambda rval: rval & 2 >> 6),
	('pid2_diff_d_en',	REG_LIA_ENABLES,	lambda s, old: (old & ~2**7) | int(s),
											lambda rval: rval & 2 >> 7),
	('pid1_diff_p_en',	REG_LIA_ENABLES,	lambda s, old: (old & ~2**8) | int(s),
											lambda rval: rval & 2 >> 8),
	('pid2_diff_p_en',	REG_LIA_ENABLES,	lambda s, old: (old & ~2**9) | int(s),
											lambda rval: rval & 2 >> 9),
	('pid1_diff_i_en',	REG_LIA_ENABLES,	lambda s, old: (old & ~2**10) | int(s),
											lambda rval: rval & 2 >> 10),
	('pid2_diff_i_en',	REG_LIA_ENABLES,	lambda s, old: (old & ~2**11) | int(s),
											lambda rval: rval & 2 >> 11),
	('pid1_bypass',	REG_LIA_ENABLES,		lambda s, old: (old & ~2**12) | int(s),
											lambda rval: rval & 2 >> 12),
	('pid2_bypass',	REG_LIA_ENABLES,		lambda s, old: (old & ~2**13) | int(s),
											lambda rval: rval & 2 >> 13),
	('lo_reset',	REG_LIA_ENABLES,		lambda s, old: (old & ~2**14) | int(s),
	...
	('pid2_diff_d_gain',	REG_LIA_DIFF_DGAIN,
											lambda s, old: (old & ~0xFFFF0000) | _sgn(s / 2**15-1,16), 
											lambda rval: _upsgn((rval & 0xFFFF0000) * 2**15-1), 16),
	...
	('frequency_demod', 	(REG_LIA_FREQDEMOD_H, REG_LIA_FREQDEMOD_L),
											lambda s, old: ((old & ~0xFFFF0000) | _usgn(s / _LIA_FREQSCALE, 48) >> 32 , _usgn(s / _LIA_FREQSCALE, 48) & 0xFFFFFFFF),
											lambda rval: _LIA_FREQSCALE * ((rval[0] & 0x0000FFFF) << 32 | rval[1])),
	('phase_demod', 		(REG_LIA_PHASEDEMOD_H, REG_LIA_PHASEDEMOD_L),
											lambda s, old: ((old & ~0xFFFF0000) | _usgn(s / _LIA_PHASESCALE, 48) >> 32 , _usgn(s / _LIA_PHASESCALE, 48) & 0xFFFFFFFF),
	...
	('sineout_amp',			REG_LIA_SINEOUTAMP,
											lambda s, old: (old & ~0x0000FFFF) | s * _LIA_AMPSCALE,
