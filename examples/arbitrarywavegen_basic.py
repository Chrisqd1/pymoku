...
stairs = [0]*8191 + [1]#np.linspace(-1,1,21)
pulse = [-1,1]
...
try:
	...
	i.write_lut(1, stairs, mode=1000)
	i.write_lut(2, pulse)

	# Prepare the waveforms (but don't enable the outputs yet)
	i.gen_waveform(1, period=1, amplitude=1.0, interpolation=False, dead_time=2.0, en=False) # Waveform output
	i.gen_waveform(2, period=0.5, amplitude=1.0, interpolation=False, en=False)	# Trigger pulse output

	# Configure the Output Channel 1 waveform to trigger off Input Channel 1 rising edge
	# Note that the triggering functionality is still disabled at this stage.
	#i.set_waveform_trigger(1, 'in1', 'rising', 0)

	# Enable the triggering functionality on Output Channel 1
	#i.set_waveform_trigger_output(1, duration=0.2) # Enables triggering

	# Enable both the outputs
	i.enable_output(ch=None) 

	"""
	i.set_waveform_trigger(1, 'in1', 'rising', 0.1) # Trigger on ADC1

	i.set_waveform_trigger_output(1, single=True, duration=2.0, hold_last=True)
	#i.sync_phase()
